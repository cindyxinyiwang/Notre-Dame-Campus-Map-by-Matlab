...
% Last Modified by GUIDE v2.5 23-Apr-2014 16:35:15
...
if getappdata(gcf,'RoutedYet')==1
    NodeList = get(handles.RouteList,'String');
...
if get(handles.AStarRadio,'Value')
...
elseif get(handles.RandomWalkRadio,'Value')
...
% give direction 
instruction = Direction(Map, Route);
for i = 1: numel(instruction)
    disp(instruction(i));
end
% get distance
distance = 0;
for index = 1:(numel(Route)-1)
    distance = distance+Map.(Route{index}).children.(Route{index+1}).cost;
end
set(handles.distanceText,'String',num2str(distance));
set(handles.RouteList,'String',Route)
DisplayBackground(handles.Map)
DisplayMap(Map,handles.Map)
